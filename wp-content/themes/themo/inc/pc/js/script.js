...
(function ($, pc) {
    ...
    var pcLinkClick = function (e) {
        ...
        if (href.indexOf('#') === 0) {
            ...
            window.open($(this).data('link'), $(this).data('target') ? '_blank' : '_self');
        }
    }
                        ...
                        switch (nodes[j].type) {
                            case 'image':
                                ...
                                var layer = $('<div/>', {
                                    ...
                                    'class': "pc-layer image " + (nodes[j].extraClass || ''),
                                }).prependTo(target);

                                if (nodes[j].link && nodes[j].link != '') {
                                    layer.data('link', nodes[j].link);
                                    layer.data('target', nodes[j].linkTargetBlank);
                                    layer.on('click', pcLinkClick);
                                }
                            ...
                            case 'nullobject':
                                ...
                                var layer = $('<div/>', {
                                    'data-id': nodes[j].id,
                                    'class': "pc-layer nullobject " + (nodes[j].extraClass || ''),
                                }).prependTo(target);
                            ...
                            case 'multi':
                                ...
                                var layer = $('<div/>', {
                                    ...
                                    'class': "pc-layer multi " + (nodes[j].extraClass || ''),
                                }).prependTo(target);
                            ...
                            case 'vc': //TAG vc
                                ...
                                if (layer) {
                                    ...
                                    layer.addClass(nodes[j].extraClass || '');
                            ...
                            case 'text':
                                ...
                                var layer = $('<div/>', {
                                    'data-id': nodes[j].id,
                                    'class': "pc-layer text " + (nodes[j].extraClass || ''),
                                    'html': nodes[j].text
                                }).prependTo(target);
                            ...
                            case 'text-block':
                                ...
                                var layer = $('<div/>', {
                                    'data-id': nodes[j].id,
                                    'class': "pc-layer text-block " + (nodes[j].extraClass || '') + " " + (nodes[j].alignHorizontal || '') + " " + (nodes[j].alignVertical || ''),
                            ...
                            case 'shape':
                                ...
                                var layer = $('<canvas/>', {
                                    'id': nodes[j].id,
                                    'data-id': nodes[j].id,
                                    'class': "pc-layer shape " + (nodes[j].extraClass || ''),
    ...
    var setFontsLink = function () {
        ...
        if (family && ext) {
            ...
            gf.attr({
                ...
                "rel": "stylesheet",
                "type": "text/css",
                "href": "//fonts.googleapis.com/css?family=" + family + "&subset=" + ext.join(',')
            });
        }
    }

    var initContactForm = function () {
        $('.pc-clone .wpcf7 > form').each(function () {
            var $form = $(this);
            wpcf7.initForm($form);
            if (wpcf7.cached) {
                wpcf7.refill($form);
    ...
    var onResizeDone = function () {
        ...
        $('.vc_page_section.parallax').each(function (i, el) {
            if ($(window).height() > $(el).outerHeight()) {
                $(el).find('.pc-r2s').height($(el).outerHeight());
            } else {
                $(el).find('.pc-r2s').height('');
            }
        });
