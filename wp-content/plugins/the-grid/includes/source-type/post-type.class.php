...
class The_Grid_Post_Type {
	...
	*/
	public function post_query() {
		...
		$gallery_img  = (count($gallery_img) == 1 && $gallery_img[0] == 0) ? null : array_filter($gallery_img);
		...
		$this->grid_query_args = array(
			...
			'paged'            => $paged,
			'offset'           => $offset,
			'post__in'         => array_filter($post_in),
			'post__not_in'     => array_filter($post_not_in),
			'order'            => $post_order,
	...
	*/
	public function image_data($img_ID) {
		...
		if (is_numeric($img_ID)) {
			...
			$img_original      = ! empty( $img_original[0] ) ? $img_original[0] : 0;
