 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class WP_IdeoImport extends WP_Importer {
	...
	// url mapping from new to old
	var $files_mapping = array();

	var $fetch_attachments = true;
	var $url_remap = array();
	var $featured_images = array();

	var $custom_post_meta = array(
		'_menu_item_anchor',
		'_menu_item_link',
		'_menu_item_mega_menu',
		'_menu_item_icon',
		'_menu_item_background',
	);

	// value of post imported previously
	var $restart_count = 0;

	// number of posts to import in one request
	const REQUEST_POSTS_LIMIT = 20;
	...
	function dispatch() {
		...
		switch ( $step ) {
			...
			case 2:
				...
				$this->fetch_attachments = true;//( ! empty( $_POST['fetch_attachments'] ) && $this->allow_fetch_attachments() );
...
<?php
	...
	/**
	 * Returns array of terms mapping (pairs old-new terms ids)
	 *
	 * @return array Key = old term id (id in import data), value = new id in db
	 */
	...
	/**
	 * Returns array of posts mapping (pairs old-new posts ids)
	 *
	 * @return array Key = old post id (id in import data), value = new id in db
	 */
	...
	function process_posts() {
		...
		$posts_count = sizeof($this->posts);
		$request_posts = 0;

		for ( $i = $this->restart_count; $i < $posts_count; $i++){
			$request_posts++;

			if ($request_posts > self::REQUEST_POSTS_LIMIT) {
				// If posts in request limit reached store restart count and finish
				$this->restart_count = $i;
				return false;
			}

			$post = apply_filters( 'wp_import_post_data_raw', $this->posts[$i] );
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( 'attachment' == $postdata['post_type'] ) {
					...
					$comment_post_ID = $post_id = $this->process_attachment( $postdata, $remote_url, $post['postmeta'] );
				} else {
			...
			if ( ! empty( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						if($key == '_ideo_post') {
							$d = preg_replace_callback('!s:(\d+):"(.*?)";!', function($match){
						...
						if($key == '_ideo_post') {
							...
							global $wpdb;
							$results = $wpdb->get_results(sprintf('INSERT INTO `%spostmeta` (`post_id`, `meta_key`, `meta_value`) VALUES (\'%s\', \'%s\', \'%s\')', $wpdb->prefix, $post_id, '_ideo_post', $value), OBJECT);
							continue;
						}
						*/
	...
	function process_menu_item( $item ) {
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			foreach($this->custom_post_meta as $custom_meta) {
				if(isset($$custom_meta))
					update_post_meta($id, $custom_meta, sanitize_key($$custom_meta));
			}
	...
	function get_attachment_metadata($post_meta) {
		if (empty($post_meta))
			return null;
	 ...
	 */
	function process_attachment( $post, $init_url, $post_meta ) {
		...
		if (!empty($this->files_mapping[$url]))
			$url = $this->files_mapping[$url];
		else
			$url = str_replace($this->url_new, $this->url_old, $url);
		...
		$upload = array(
			'error' => false,
			'url' => $init_url,
			'file' => $upload_dir['basedir'] . substr($init_url, strlen($upload_dir['baseurl']))
		);
		...
		$attachment_metadata = $this->get_attachment_metadata($post_meta);
		...
		if ( preg_match( '!^image/!', $info['type'] ) ) {
			...
			{
				// Add mapping information for every image size
				foreach($attachment_metadata['sizes'] as $size)
					$this->url_remap[$parts['dirname'] . '/' . $size['file']] = $parts_new['dirname'] . '/' . $size['file'];
			}
		}
...
function wordpress_ideoimporter_init() {
	load_plugin_textdomain( 'wordpress-ideoimporter', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );

	/**
	 * WordPress Importer object for registering the import callback
	 * @global WP_Import $wp_import
	 */
	$GLOBALS['wp_ideoimport'] = new WP_IdeoImport();
	//register_importer( 'wordpress', 'Wordpress', __('Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', 'wordpress-ideoimporter'), array( $GLOBALS['wp_ideoimport'], 'dispatch' ) );
}
add_action( 'admin_init', 'wordpress_ideoimporter_init' );
