...
vc_map(array(
    'name' => __('Icon Box', 'ideo-themo'),
    'base' => 'ideo_iconbox',
    'icon' => 'icon-icon-box-1',
    ...
    'params' => array(
        ...
        array(
            'type' => 'ideo_slider',
            'heading' => __('ICON BORDER THICKNESS(px)', 'ideo-themo'),
        ...
        array(
            ...
            'std' => ideothemo_get_shortcodes_default_style('ideo_iconbox'),
...
$el_type = $el_icon_align = $el_icon_circle_border_size = $el_title = $el_title_size = $el_text_size = $el_text_align = $el_icon_type = $el_icon_font = $el_icon_font_size = $el_icon_custom = $el_button_display = $el_button_label = $el_button_link = $el_button_type = $el_button_radius = $el_button_border_thickness = $el_button_size = $el_button_align = $el_button_icon_type = $el_button_icon = $el_button_icon_position = $el_margin_top = $el_margin_bottom = $el_extra_class = $el_elemnt_style = $el_elemnt_style_colors = $el_button_elemnt_style = $el_button_elemnt_style_colors = $el_animation = $el_animation_type = $el_animation_delay = $el_animation_duration = $el_animation_offset = $el_uid = $content = '';

function ideothemo_iconbox_func($atts, $content = "")
{
    ...
    extract(shortcode_atts(array(
        ...
        'el_margin_bottom' => '20',
        'el_extra_class' => '',
        'el_elemnt_style' => ideothemo_get_shortcodes_default_style('ideo_iconbox'),
    ...
    if ($el_margin_top != '') {
        if ($el_icon_align == 'center' && $el_type == 'big-icon') {
            $less .= 'margin-top:' . (int)($el_margin_top + 100) . 'px;';
        } else {
    ...
    if ($el_type == 'small-icon' && $el_icon_font_size) {
        $less .= '.icon i{font-size:' . ideothemo_get_size($el_icon_font_size) . ';}';
        $less .= '&.align-icon-left{padding-left: 28+2*' . ideothemo_get_size($el_icon_font_size) . ';}';
        $less .= '&.align-icon-right{padding-right: 28+2*' . ideothemo_get_size($el_icon_font_size) . ';}';
    }
    ...
    $colors_array = array_replace_recursive($colors, array_filter($colors_array));


    $less .= '.icon{';
    $less .= 'border-width:' . $el_icon_circle_border_size . 'px';
    $less .= '}';
    ...
    $html .= '<span class="icon"><i class="' . esc_attr(isset($el_icon_font) && $el_icon_type == 'font' ? $el_icon_font : '') . '"></i></span>';
...
add_shortcode('ideo_iconbox', 'ideothemo_iconbox_func');
