...
if (!function_exists('foton_mikado_core_plugin_message')) {
    /**
     * Function that prints a mesasge in the admin if user hides TGMPA plugin activation message
     */
    function foton_mikado_core_plugin_message()
    {
        if (get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice', true) && !foton_mikado_core_plugin_installed()) {
            echo apply_filters('foton_mikado_filter_core_plugin_message', '<div class="update-nag">' . esc_html__('Installation of the "Mikado Core" plugin is essential for proper theme functioning. Please ', 'foton') . '<a href="' . esc_url(admin_url('themes.php?page=install-required-plugins')) . '">' . esc_html__('install', 'foton') . '</a>' . esc_html__(' this plugin and activate it', 'foton') . '</div>');
        }
    }

    add_action('admin_notices', 'foton_mikado_core_plugin_message');
}
...
if (!function_exists('foton_mikado_paged_home')) {
    /**
     * Set false to on paged url on homepage
     *
     * @return int
     */
    function foton_mikado_paged_home()
    {
        global $wp_query;
    ...
    add_filter('redirect_canonical', 'foton_mikado_paged_home');
}
