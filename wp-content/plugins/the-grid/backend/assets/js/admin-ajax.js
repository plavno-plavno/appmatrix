...
// ==================================================================
	...
	var current_post,
		...
		skin_data_arr  = JSON.parse( $('.tomb-grid-skins').val() || '{}' );//$.parseJSON($('.tomb-grid-skins').val());
	...
	if ($('#tg-grid-skins').length > 0) {
		...
		var grid_skins_preview = function() {
			...
			$('.tomb-post-type-skin').on('change', function() {
				update_selected_skin();
				$('#tg-grid-skins .tg-grid-wrapper:not(.skin-hidden) .tg-filter-active').trigger( 'click' );
			});
...
// ==================================================================
			...
			callbacks : {
				...
				success : function(ajax_data, response, msg) {
					if (response.content) {
						...
						grid_preview_width();
						TG_excludeItem();
						//$grid_preloader_styles.attr('scoped', false);
	...
	$grid_preview_viewport.find('div').on( 'click', function() {
