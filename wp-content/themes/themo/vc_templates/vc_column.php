...
switch ($el_background) {
    case 'color':
        ...
        if ($el_background_color_pattern) $less .= 'background-image:url(' . ideothemo_get_assets_svg_data('svg/masks/' . $el_background_color_pattern . '.svg', $el_background_color_pattern_color) . ');';
...
if ($el_url && $el_url !== '|||') {
    ...
    $html .= '<a class="vc_column_container_link" href="' . esc_url($a_href) . '"  title="' . esc_attr($a_title) . '" target="' . esc_attr($a_target) . '"></a>';
}
