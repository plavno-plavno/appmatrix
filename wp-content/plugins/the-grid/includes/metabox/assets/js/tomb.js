...
var TOMB_ImageSelect = {
	event: function() {
		jQuery(document).on('click', '.tomb-image-holder', function(){
			var $this = jQuery(this);
			$this.closest('.tomb-field').find('.tomb-image-holder, input, img').removeAttr('data-checked');
			$this.closest('.tomb-field').find('.tomb-image-holder, input, img').attr('checked', false);
...
(function($){
	$.fn.textareaCount = function(options, fn) {
		...
		container.on('keyup', function(){limitTextAreaByCharacterCount();})
				 .on('mouseover', function(){setTimeout(function(){limitTextAreaByCharacterCount();}, 10);})
				 .on('paste', function(){setTimeout(function(){limitTextAreaByCharacterCount();}, 10);});
...
jQuery.expr.pseudos.TOMB_hasScroll = TOMB_hasScroll;
	...
	$.fn.To_linenumbers = function(in_opts){
		...
		return this.each(function(){
			...
			if (!el.hasClass('tomb-code-init')) {
				...
				code.on('blur focus change keyup keydown',function(){
